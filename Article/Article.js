...
function componentFactory(data) {
  ...
  const paragraphs = [];
  // structure shows span but couldn't get to work without button
  ...
  // append elements to parent article div
  article.appendChild(header);
  article.appendChild(date);
  paragraphs.forEach(paragraph => article.appendChild(paragraph));
  article.appendChild(button);

  // add classes to classList of elements
  ...
  paragraphs.forEach((paragraph, index) => {
    paragraph.textContent = data[paragraphKeys[index]];
  });

  // add eventListener to button element
  button.addEventListener("click", event => {
    article.classList.toggle("article-open");
    // change height attribute to auto on article open; 50px if not open
...
data.map(object => {
  articlesContainer.appendChild(componentFactory(object));
});
